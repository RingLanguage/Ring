...
void ring_compiler_analysis_function(Package* package) {
    ...
    for (Function* function : package->function_list) {
        ...
        if (function->type == FUNCTION_TYPE_NATIVE && function->block != nullptr) {
            // Ring-Compiler-Error-Report ERROR_NATIVE_FUNCTION_PROVIDE_BLOCK
            DEFINE_ERROR_REPORT_STR;

            compile_err_buf = sprintf_string(
                "function `%s` is native function, no need provide code block; E:%d.",
                function->identifier,
                ERROR_NATIVE_FUNCTION_PROVIDE_BLOCK);
            ...
            ring_compile_error_report(&context);
        } else if (function->type == FUNCTION_TYPE_DERIVE && function->block == nullptr) {
