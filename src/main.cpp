...
Ring_Command_Arg ring_parse_command(int argc, char** argv) {
    ...
    // -e --escape-strings
    // 开始这个选项，可以在 ring dump 的过程中，将string 常量中的字符进行转义，这样显示更加规整
    auto escape_string_rule = (clipp::option("-e", "--escape-strings").set(escape_strings, true).doc("escape strings in dump"));
