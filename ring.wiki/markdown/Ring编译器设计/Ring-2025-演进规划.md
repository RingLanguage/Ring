...
1. len的数据类型 int/itn64/double
2. 是否可以控制步长
3. 是否可以 反向便利
4. 同时优化目前 的range 字节码的设计，感觉设计有点复杂
   1. 之前使用了 jump 字节码，这里需要单独提供一个


for (start = range 0..len..step) {
...
for (value = range []string{}) {

}

```json
// 在 RVM_Opcode_Infos 数组中添加
{NEW_CODE_RANGE_INIT, "range_init", OPCODE_OPERAND_TYPE_3BYTE_AsB, "...", ..., "init range from start to end", "...", "..."},
{NEW_CODE_RANGE_INC, "range_inc", OPCODE_OPERAND_TYPE_1BYTE_A, "...", ..., "increment range counter", "...", "..."},
{NEW_CODE_RANGE_COND, "range_cond", OPCODE_OPERAND_TYPE_2BYTE_As, "...", ..., "check range condition", "...", "..."},
{NEW_CODE_RANGE_JUMP, "range_jump", OPCODE_OPERAND_TYPE_2BYTE_As, "...", ..., "jump to range loop start", "...", "..."},
```


```
PUSH_INT 0          ; 压入起始值 0
PUSH_INT len        ; 压入结束值 len
loop_start:
RANGE_COND          ; 判断是否继续循环
JUMP_IF_FALSE end   ; 条件不满足则跳出循环
; 循环体字节码...
RANGE_INC 1         ; 计数器加 1
JUMP loop_start     ; 跳转到循环开始
end:
```
...
只实现  array/class_object/1..len 这种
...
for 语言，尝试去掉 括号
在 bison中，将 token_assign token_range 合为一个
