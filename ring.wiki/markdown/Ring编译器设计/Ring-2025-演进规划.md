...
## 22. Proposal: 优化 reflect 标准库

支持
这里有个依赖，就是native函数需要支持返回class-object


reflect::get_obj_fields(var any class_obj) -> (strings[]);

reflect::get_obj_field_value(var string file_name) -> (any);
...
## 36. Proposal: 添加右移，左移运算

## 37. Proposal: and or not 替换成 这种 && || !

## 38. Proposal: 支持按位 取反 与 或 异或 运算
